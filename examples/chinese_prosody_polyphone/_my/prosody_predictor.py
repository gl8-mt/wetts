...
def _combine_prosody(text, rank_lst, pred_rank_lst, min_dist=4):
    ...
    for pos, _ in rank_lst:
        for i in range(max(0, pos - min_dist), pos + min_dist):
            if i in d:
                logging.info(f'ignore prosody nearby artifact pos {pos}: {i} {d[i]}')
                del d[i]
...
class Frontend(object):
    ...
    def __init__(
        ...
        self.sess = runtime_cls(polyphone_prosody_model, pretrain_bert_model, device=device)
...
class ProsodyPredictor:
    ...
    def __init__(self, config: dict = None):
        ...
        self.frontend = Frontend(
                config['model_path'],
                config['pretrain_bert_model'],
                device=config.get('device', 'cpu')
        )
        return

    def predict(self, text: str, use_sp: bool=False, use_postprocess=True, use_artifact=False) -> str:
        """
        摩尔线程真棒 => 摩尔线程 sp0 真棒
